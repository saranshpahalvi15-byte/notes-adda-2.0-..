...
import { LogOut, User, LayoutDashboard, Settings, Menu, X, Target, Gift, Moon, Sun, Flame, CheckCircle, Sparkles } from 'lucide-react';
...
export default function Layout() {
  const { user, profile, logout, notification, setNotification } = useAuthStore();
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100 transition-colors duration-300">
      <StudyTracker />
        ...
        {/* Toast Notification */}
        {notification && (
          <div className="fixed top-20 right-4 z-[100] animate-in fade-in slide-in-from-right-4 duration-300 w-full max-w-sm">
            <div className={`px-6 py-3 rounded-xl shadow-2xl flex items-center gap-3 backdrop-blur-md border ${
              ...
              {notification.type === 'success' ? <CheckCircle className="h-5 w-5 text-green-200" /> : notification.type === 'error' ? <X className="h-5 w-5 text-red-200" /> : <Sparkles className="h-5 w-5 text-indigo-200" />}
        ...
        <div className="bg-[#25D366] text-white px-4 py-2 text-sm font-bold text-center flex justify-center items-center">
            <a 
              href="https://whatsapp.com/channel/0029Vb8UtXW3QxRvaMKkyM19" 
              target="_blank" 
        ...
        <header className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 sticky top-0 z-50 transition-colors">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between h-16 items-center">
              <div className="flex items-center">
                <button 
                  className="lg:hidden mr-2 p-2 rounded-md text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-800 focus:outline-none"
                ...
                <Link to="/" className="flex items-center space-x-1 sm:space-x-2">
                  ...
                  <span className="text-xl md:text-2xl font-black tracking-tight whitespace-nowrap" style={{ fontFamily: "'Nunito', 'Arial Rounded MT Bold', sans-serif" }}>
                    <span className="text-[#0B406B] dark:text-[#3482c3]">Notes</span>
              ...
              <nav className="hidden lg:flex space-x-1">
                <Link to="/notes" className="text-xs xl:text-sm px-2 xl:px-3 py-2 rounded-md text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-indigo-600 dark:hover:text-indigo-400 font-medium transition-colors">
                  Chapter Wise Notes
                </Link>
                <Link to="/bundles" className="text-xs xl:text-sm px-2 xl:px-3 py-2 rounded-md text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-indigo-600 dark:hover:text-indigo-400 font-medium transition-colors">
                  Bundles
                </Link>
                <Link to="/mindMaps" className="text-xs xl:text-sm px-2 xl:px-3 py-2 rounded-md text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-indigo-600 dark:hover:text-indigo-400 font-medium transition-colors">
                  Mind Maps
                </Link>
                ...
                <Link to="/giveaways" className="text-sm px-3 py-2 rounded-md text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-indigo-600 dark:hover:text-indigo-400 font-medium transition-colors flex items-center">
                  <Gift className="h-4 w-4 mr-1.5 text-indigo-500 dark:text-indigo-400" />
                  Giveaways
                </Link>
              </nav>
                ...
                {user ? (
                  <div className="flex items-center space-x-2 sm:space-x-4">
                    <button
                      ...
                      className="hidden sm:flex items-center px-3 py-1.5 border border-indigo-200 dark:border-indigo-800 rounded-lg text-sm font-medium text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-900/30 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors"
                    >
                      <Target className="h-4 w-4 mr-1.5 text-indigo-500 dark:text-indigo-400" />
                      Goal: {currentGoalLabel}
                    </button>
                    {profile?.streakCount !== undefined && profile.streakCount > 0 && (
                      <div className="hidden sm:flex items-center px-3 py-1.5 bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400 rounded-lg text-sm font-bold shadow-sm" title="Day Study Streak!">
                        <Flame className="h-4 w-4 mr-1 text-orange-500 fill-orange-500 animate-pulse" />
                        {profile.streakCount}
                      </div>
                    )}
                    {profile?.role === 'admin' && (
                      <Link to="/admin" className="text-gray-500 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 flex items-center p-2 sm:p-0">
                        <Settings className="h-5 w-5 sm:mr-1" />
                        <span className="hidden lg:inline text-sm font-medium">Admin</span>
                      </Link>
                    )}
                    <Link to="/dashboard" className="text-gray-500 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 flex items-center p-2 sm:p-0">
                      <LayoutDashboard className="h-5 w-5 sm:mr-1" />
                      <span className="hidden lg:inline text-sm font-medium">Dashboard</span>
                    </Link>
                    <button
                      onClick={handleLogout}
                      className="text-gray-500 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-400 flex items-center p-2 sm:p-0"
                    >
          ...
          {/* Mobile menu */}
          {isMobileMenuOpen && (
            <div className="lg:hidden bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 px-2 pt-2 pb-3 space-y-1 sm:px-3 shadow-inner w-full">
              {user && (
                <button
                  onClick={() => {
                  ...
                  className="w-full flex items-center px-3 py-2 rounded-md text-base font-medium text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-900/30 mb-2"
                >
        ...
        <footer className="bg-[#99c2c6] dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 mt-auto">
          <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                ...
                <div>
                  <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100 tracking-wider uppercase mb-4">Quick Links</h3>
                  <ul className="space-y-3">
                    <li><Link to="/notes" className="text-sm text-gray-600 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400">Chapter Wise Notes</Link></li>
                ...
                <div>
                  ...
                  <ul className="space-y-3">
                    ...
                    <li><Link to="/contact" className="text-sm text-gray-600 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400">Contact Us</Link></li>
                    <li><Link to="/privacy" className="text-sm text-gray-600 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400">Privacy Policy</Link></li>
                    <li><Link to="/terms" className="text-sm text-gray-600 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400">Terms of Service</Link></li>
                    <li><Link to="/refund" className="text-sm text-gray-600 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400">Refund Policy</Link></li>
                  </ul>
                </div>
                ...
                <div>
                  ...
                  <div className="flex space-x-4">
                    <a href="https://whatsapp.com/channel/0029Vb8UtXW3QxRvaMKkyM19" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-[#25D366]">
                      <span className="sr-only">WhatsApp Connection</span>
